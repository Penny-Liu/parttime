...
const App: React.FC = () => {
  ...
  useEffect(() => {
    if (selectedAdminDate && data) {
      setAdminNote(data.shifts[selectedAdminDate]?.note || '');
    } else {
      setAdminNote('');
    }
  ...
  const handleSaveNote = async () => {
    ...
    const newShift = { ...shift, note: adminNote };
  ...
  const handleAdminShiftAction = async (action: 'confirm' | 'close' | 'clear' | 'no_student', targetUserId?: string) => {
    if (!selectedAdminDate || !data) return;
    ...
    } else if (action === 'no_student') {
      newShift.confirmedUserId = 'NO_STUDENT';
      newShift.isClosed = false;
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden transform scale-100 transition-all">
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-3 gap-3 pt-4 border-t border-gray-100">
                 <button 
                    ...
                    className={`p-3 rounded-xl font-bold border-2 transition-all ${
                      data.shifts[selectedAdminDate]?.isClosed 
                 ...
                 <button 
                    onClick={() => handleAdminShiftAction('no_student')}
                    className={`p-3 rounded-xl font-bold border-2 transition-all ${
                      data.shifts[selectedAdminDate]?.confirmedUserId === 'NO_STUDENT'
                      ? 'bg-gray-500 text-white border-gray-500 shadow-lg shadow-gray-200' 
              ...
              <div className="pt-4 border-t border-gray-100">
                <h4 className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-2">備註事項</h4>
                <div className="flex gap-2">
                  <input
                    type="text"
                    value={adminNote}
                    onChange={e => setAdminNote(e.target.value)}
                    placeholder="輸入當日備註 (如: 早上不排班)"
                    className="flex-1 p-3 border border-gray-200 rounded-xl text-sm outline-none focus:border-blue-300 focus:ring-2 focus:ring-blue-100 transition-all"
                  />
                  <button
